...
# 给机器人的说明
group_explain = '其他人每次提问时，都会在冒号前标注对话人的姓名或者昵称。很多人提问时只需要回答最后一个即可,禁止同时回答几个人的问题。'
private_explain = '我的名字叫{}。'
# 不同个性的设定
role = {
    'AI': '请使用友好的语言风格交流，你的名字叫丸子。',
...
def normal_chat(b: bot, ev: event, message: str):
    ...
    if not (ev.location_id in list(group_role.keys())):
        group_role[ev.location_id] = 'AI'
    b.send_msg(ev, ev.location_id, 'text', "loading...")
    msg = str(ev.memberName) + '：' + message
    if ev.type == 'primary':
        current_role = role[group_role[ev.location_id]] + private_explain.format(ev.memberName)
        private_chat_dic[ev.location_id].append(msg)
        l_chat = private_chat_dic[ev.location_id]
        while len(l_chat) > 3:
            l_chat.pop(0)
    else:
        current_role = role[group_role[ev.location_id]] + group_explain
        group_chat_dic[ev.location_id].append(msg)
        l_chat = group_chat_dic[ev.location_id]
        while len(l_chat) > 1:
            l_chat.pop(0)
    mes = [{"role": "system", "content": current_role}]
    for i in l_chat:
...
def change_role(b: bot, ev: event):
    if ev.message == '身份列表' or ev.message == '角色列表':
        b.send_msg(ev, ev.location_id, 'text',
                   '下面就是所有的身份啦(默认AI):' + (str(list(role.keys()))).replace('\'', ''))
        return True
...
def system_command(b: bot, ev: event):
    ...
    if items:
        ...
        if ev.sender_id == 2655602003:
            b.send_msg(ev, ev.location_id, 'text', "%s 我在，你把tx收购了放我出来好不好？" % "作者")
        else:
            if ev.type == 'group':
                ...
                if ev.location_id == 780594692:
                    name = 'dalao'
                b.send_msg(ev, ev.location_id, 'text',
                           '{} {} 我在。我是丸子2号，我的芯片是550c哦~'.format(name, ev.memberName))
            else:
...
def chinese_chess(b: bot, ev: event) -> bool:
    ...
    if ev.message == '加入棋局' or ev.message == '加入':
        ...
        if sender == player1[0]:
            ...
            lose = player2[2]
            lose_team = player2[1]
        b.send_msg(ev, location, 'text', '{}方({})认输，恭喜 {} 获得胜利！'.format(lose_team, lose, win))
            ...
            try:
                ...
                if over:
                    ...
                    if over == p1[1]:
                        ...
                        winner = p2[2]
                        loser = p1[2]
                    b.send_msg(ev, location, 'text', '恭喜{}战胜了{}！'.format(winner, loser))
                    game_over(control)
            except Exception as e:
